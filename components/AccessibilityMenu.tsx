...
function Toggle({ label, checked, onChange }: ToggleProps) {
  return (
    ...
    >
      <span>{label}</span>
      <span className={`relative inline-flex h-6 w-11 items-center rounded-full transition ${checked ? 'bg-cyan-400/80' : 'bg-slate-500/50'}`} aria-hidden>
...
export default function AccessibilityMenu() {
  ...
  return (
    <div className="fixed bottom-5 start-5 z-50">
      ...
      <aside className={`mt-3 w-72 rounded-2xl border border-white/15 bg-slate-950/85 p-4 shadow-[0_20px_50px_rgba(8,15,35,0.65)] backdrop-blur-2xl transition duration-200 ${open ? 'pointer-events-auto scale-100 opacity-100' : 'pointer-events-none scale-95 opacity-0'}`}>
        <h2 className="text-sm font-semibold uppercase tracking-wide text-cyan-100">{t.accessibility.title}</h2>
        <div className="mt-3 space-y-2">
          <Toggle label={t.accessibility.largerText} checked={largeText} onChange={setLargeText} />
          <Toggle label={t.accessibility.contrast} checked={highContrast} onChange={setHighContrast} />
          <Toggle label={t.accessibility.reduceMotion} checked={reduceMotion} onChange={setReduceMotion} />
